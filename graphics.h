...
extern Image* hubAllocForImage(const char* filename);
extern void hubLoadImage(const char* filename, Image *image);
...
//extern void blitAlphaImageToImage2(int sx, int sy, int width, int height, Image* source, int dx, int dy, Image* destination);
...
//extern void blitAlphaImageToScreen(float sx, float sy, float width, float height, Image *img,float x, float y);
/**
