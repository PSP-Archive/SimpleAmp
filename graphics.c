...
Image* hubAllocForImage(const char* filename)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
void hubLoadImage(const char* filename, Image *image)
{
	png_structp png_ptr;
	png_infop info_ptr;
	unsigned int sig_read = 0;
...
Image* loadImage(const char* filename)
{
	...
	if ((fp = fopen(filename, "rb")) == NULL)
	  errorDisplay("Graphics engine error\n\nCouldn't open file :\n",filename);
	png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	if (png_ptr == NULL) {
		free(image);
		fclose(fp);
	...
	image->textureWidth = getNextPower2(width);
	image->textureHeight = getNextPower2(height);
	image->totalSize = image->textureWidth * image->textureHeight * sizeof(u32);
	png_set_strip_16(png_ptr);
	png_set_packing(png_ptr);
	if (color_type == PNG_COLOR_TYPE_PALETTE) png_set_palette_to_rgb(png_ptr);
	if (color_type == PNG_COLOR_TYPE_GRAY && bit_depth < 8) png_set_gray_1_2_4_to_8(png_ptr);
	...
	fclose(fp);
	// Swizzle the texture
	/*int rowblocks = ((image->textureWidth * 4)/16);
	int rowblocks_add = (rowblocks-1) * 128;
	unsigned int block_address = 0;
	unsigned int *src = (unsigned int*)image->data;
	u8* temp;
	//if(location) Dream_Team : VRAM to be added later
 	//	temp = (u8*)valloc(image->totalSize);
	//else
		temp = (u8*)malloc(image->totalSize);

	int j;
	for(j=0; j<image->textureHeight; j++, block_address+=16) {
		unsigned int *block = (unsigned int*)((unsigned int)&temp[block_address]|0x40000000);
		int i;
	...
	//vfree(image->data); Dream_Team : VRAM to be added later
	free(image->data);
	image->data = (u32*)temp;*/
	return image;
}
  ...
  unsigned int alpha;
	...
	for (y = 0; y < height; y++, destinationData += destinationSkipX, sourceData += sourceSkipX) {
      ...
      if (alpha != 0) 
      {
        ...
        *destinationData = (color & 0xff000000) | ((rb | g) >> 8) | 0xff000000;       
      }	
...
void imageDrawImageSectionAlpha(int inStartX, int inStartY, int inWidth, int inHeight, Image* inSource, int inDestX, int inDestY, Image* inDestination) {
     Color* tempDestinationData = &inDestination->data[(inDestination->textureWidth * inDestY) + inDestX];
     int tempDestinationSkipX = inDestination->textureWidth - inWidth;
     Color* tempSourceData = &inSource->data[inSource->textureWidth * inStartY + inStartX];
     int tempSourceSkipX = inSource->textureWidth - inWidth;
     int x, y;
